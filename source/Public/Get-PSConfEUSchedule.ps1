...
function Get-PSConfEUSchedule {
    <#
    .SYNOPSIS
        Gets the PSConfEU Schedule from the Sessionize API

    .DESCRIPTION
        Gets the PSConfEU Schedule from the Sessionize API and outputs to json, excel, psobject, html or csv
    ...
    .EXAMPLE
        ...
        Gets the PSConfEU Schedule from the Sessionize API and outputs to excel, opens the file and saves it to the default temp directory
    ...
    .EXAMPLE
        ...
        Gets the PSConfEU Schedule from the Sessionize API and outputs as a psobject on the screen
    ...
    .EXAMPLE
        Get-PSConfEUSchedule -speaker Buck -output object | ft
...
#>
    ...
    $BaseUri = 'https://sessionize.com/api/v2/k0g4vkl5/view'
...
{1}'  -f @(
    ...
    switch ($output) {
        ...
        'Excel' {
            if (Get-Module -Name ImportExcel -ErrorAction SilentlyContinue -ListAvailable) {
                if ($filter -eq 'Schedule') {
                    ...
                    $FilePath = '{0}\PSConfEUSchedule{1}_{2}.xlsx' -f $fileDirectory, $filter, $Date
                ...
                $FilePath = '{0}\PSConfEU_{1}_{2}.csv' -f $fileDirectory, $filter, $Date
        ...
        'html' {
            $FilePath = '{0}\PSConfEU_{1}_{2}.html' -f $fileDirectory, $filter, $Date
